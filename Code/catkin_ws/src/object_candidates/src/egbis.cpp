...
cv::Mat runEgbisOnMat(const cv::Mat& input, float sigma, float k, int min_size, int *numccs, std::vector<std::vector<cv::Point>>& segments) {
    int w = input.cols;
    int h = input.rows;
	cv::Mat output(cv::Size(w,h),CV_8UC3);
    ...
    image<rgb> *segmentedImage = segment_image(nativeImage, sigma, k, min_size, numccs, segments);
